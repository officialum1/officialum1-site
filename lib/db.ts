// Database Connection Hub - v2.0 (Forced Sync with Global Cache Persistence)
import mysql from 'mysql2/promise';

// Cache connection pool strictly across hot-reloads and serverless boots to prevent ER_USER_LIMIT_REACHED
let pool: mysql.Pool;
...
if (!globalForDb.pool) {
    globalForDb.pool = mysql.createPool({
        ...
        connectionLimit: 5, // Optimized connection count for Hostinger Shared Hosting limits
        queueLimit: 0,
...
// Helper to query the database with auto-retry on ECONNRESET / dropped idle socket
export async function query(sql: string, params: any[] = []) {
    try {
        ...
        if (error?.code === 'ECONNRESET' || error?.code === 'PROTOCOL_CONNECTION_LOST' || error?.code === 'ETIMEDOUT') {
            console.warn('MySQL Connection dropped (ECONNRESET/TIMEOUT). Retrying query...');
            try {
                const [results] = await pool.query(sql, params);
                return results;
            } catch (retryErr) {
                console.error('Database Retry Query Error:', retryErr);
                throw retryErr;
            }
        }
...
export async function initDB(force = false) {
    ...
    try { await query("ALTER TABLE blogs ADD COLUMN meta_title VARCHAR(255)"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN meta_description TEXT"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN focus_keyword VARCHAR(255)"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN seo_keywords TEXT"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN canonical_url VARCHAR(500)"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN robots VARCHAR(50) DEFAULT 'index,follow'"); } catch (e) { }
    try { await query("ALTER TABLE blogs ADD COLUMN schema_type VARCHAR(50) DEFAULT 'BlogPosting'"); } catch (e) { }
    ...
    try { await query("ALTER TABLE products ADD COLUMN seo_title VARCHAR(255)"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN seo_description TEXT"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN focus_keyword VARCHAR(255)"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN seo_keywords TEXT"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN canonical_url VARCHAR(500)"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN robots VARCHAR(50) DEFAULT 'index,follow'"); } catch (e) { }
    try { await query("ALTER TABLE products ADD COLUMN schema_type VARCHAR(50) DEFAULT 'Product'"); } catch (e) { }
    ...
    await query(`
        CREATE TABLE IF NOT EXISTS market_intel (
            ...
            platform VARCHAR(50), -- 'Marketplace', 'Direct', etc.
