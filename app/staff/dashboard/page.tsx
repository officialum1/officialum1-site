...
export default function StaffDashboard() {
    ...
    const [newLead, setNewLead] = useState({ name: '', contact: '', source: 'Direct', value: '', notes: '' });
    ...
    const handleAddLead = async (e: React.FormEvent) => {
        ...
        setNewLead({ name: '', contact: '', source: 'Direct', value: '', notes: '' });
        fetchLeads(staff);
    };
        ...
        try {
            ...
            if (data.success) {
                if (data.delivery) {
                    ...
                    setSaleData({ salePrice: '', platform: 'OfficialUM1', description: '' });
                    setSelectedItem(null);
                }
                fetchInventory(staff);
            } else {
    ...
    const handleQuickSaleOpen = (item: any) => {
        setSelectedItem(item);
        setSaleData({ ...saleData, description: `Sold: ${item.name}`, platform: item.platform || 'OfficialUM1' });
                                ...
                                <div className="glass" style={{ padding: '2rem', borderRadius: '16px', border: '1px solid rgba(0,255,136,0.3)', position: 'sticky', top: '120px' }}>
                                    ...
                                    {deliveryInfo ? (
                                        <div style={{ animation: 'fadeIn 0.5s' }}>
                                            ...
                                            <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => { setDeliveryInfo(null); setShowSaleForm(false); setSaleData({ salePrice: '', platform: 'OfficialUM1', description: '' }); setSelectedItem(null); }}>Done</button>
                                        </div>
                                    ) : showSaleForm ? (
                                        <form onSubmit={handleSaleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                            ...
                                            <select className="input-field" value={saleData.platform} onChange={e => setSaleData({ ...saleData, platform: e.target.value })}><option value="OfficialUM1">OfficialUM1</option><option value="Direct">Direct</option></select>
