...
const Z2UOfferCreator = dynamic(() => import('@/components/admin/tabs/Z2UOfferCreator'), { ssr: false });
const BuilderTab = dynamic(() => import('@/components/admin/tabs/BuilderTab'), { ssr: false });
const WhatsAppTab = dynamic(() => import('@/components/admin/tabs/WhatsAppTab'), { ssr: false });
...
function AdminDashboard() {
    ...
    // Sync URL -> Tab State (Requested Fix: Link handling)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        }, 15000); // 15s Pulse - Reduced from 3s to prevent 504 timeouts on heavy DB queries
    ...
    const handleAddInventory = async (e: React.FormEvent) => {
        ...
        setShowAddInv(false);
        setNewItem({ name: '', platform: 'Z2U', purchasePrice: '', credentials: '', username: '', password: '', email: '', emailPassword: '', country: '', extraInfo: '', tag: '', image: '' });
        fetchData();
    };
    ...
    return (
        <AdminShell
            ...
            subtitle="Inventory, orders, tools, and operations in one place."
            hideSidebar
        >
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                                {[
                                    ...
                                    { id: 'z2u_creator', label: '🛰️ Z2U Log\'s', perm: 'z2u' },
                                    { id: 'whatsapp', label: '📱 WhatsApp Inbox', perm: 'orders' },
                                    { id: 'builder', label: '🛠️ Builder Requests', perm: 'orders' },
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                                {[
                                    ...
                                    { id: 'business', label: '🏢 Formations', perm: 'sales' },
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                                {[
                                    ...
                                    { id: 'live_traffic', label: '🟢 Live Traffic', perm: 'marketing' },
                                    { id: 'tools', label: '🛠️ Tools', perm: 'tools' },
                                    { id: 'kb', label: '📚 KB/FAQ', perm: 'website' },
                                    { id: 'indexing', label: '🚀 Google Indexing', perm: 'website' },
                                    { id: 'z2u_intel', label: '📈 Z2U Intel', perm: 'tools' },
                                ].filter(tab => hasPermission(tab.perm)).map(tab => (
                                    <button
                                        key={tab.id}
                                        onClick={() => {
                                            if (tab.id === 'z2u_intel') {
                                                router.push('/admin/z2u-intel');
                                                return;
                                            }
