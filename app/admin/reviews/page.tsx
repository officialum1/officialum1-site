    ...
    return (
        <main>
            ...
            <div className="container" style={{ paddingTop: '150px', paddingBottom: '100px' }}>
                ...
                <h1 style={{ marginBottom: '2rem' }}>Review Moderation 🌟</h1>
                            ...
                            {reviews.map(r => (
                                <tr key={r.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <td style={{ padding: '1rem', display: 'flex', gap: '0.5rem' }}>
                                        {r.status !== 'approved' && (
                                            <button onClick={() => handleAction(r.id, 'approve')} className="btn" style={{ padding: '5px 10px', fontSize: '0.8rem', background: '#00ff88', color: 'black' }}>✓</button>
                                        )}
                                        {r.status !== 'rejected' && (
                                            <button onClick={() => handleAction(r.id, 'reject')} className="btn" style={{ padding: '5px 10px', fontSize: '0.8rem', background: '#ffaa00', color: 'black' }}>✕</button>
                                        )}
                                        <button onClick={() => handleAction(r.id, 'delete')} className="btn" style={{ padding: '5px 10px', fontSize: '0.8rem', background: '#ff4444', color: 'white' }}>🗑</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
