...
export default function CouponsManager() {
    ...
    return (
        <AdminShell title="Coupons" subtitle="Create, edit, and manage promo codes.">
            <a href="/admin/dashboard" className="btn-ghost" style={{ marginBottom: '1rem', display: 'inline-flex' }}>← Back to Dashboard</a>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Form */}
                    <div className="card h-fit" style={{ padding: '1.5rem' }}>
                        <h3 style={{ marginBottom: '1.5rem', fontWeight: 'bold' }}>{isEditing ? 'Edit Coupon' : 'Create New Coupon'}</h3>
                        <form onSubmit={handleSave} className="space-y-4">
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Code</label>
                                <input className="input w-full" value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} required placeholder="e.g. SUMMER20" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" className="input w-full" value={form.value} onChange={e => setForm({ ...form, value: parseFloat(e.target.value) })} required />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Min. Order Amount ($)</label>
                                <input type="number" className="input w-full" value={form.min_amount} onChange={e => setForm({ ...form, min_amount: parseFloat(e.target.value) })} />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Expiry Date (Optional)</label>
                                <input type="date" className="input w-full" value={form.expiry} onChange={e => setForm({ ...form, expiry: e.target.value })} />
                            </div>
                            <div>
                            ...
                            <div className="flex gap-2 pt-4">
                                <button type="submit" className="btn-primary w-full">{isEditing ? 'Update Coupon' : 'Create Coupon'}</button>
                                {isEditing && <button type="button" onClick={() => { setIsEditing(false); setForm({ id: 0, code: '', type: 'percent', value: 0, min_amount: 0, expiry: '', status: 'active' }); }} className="btn-secondary">Cancel</button>}
                            </div>
                        </form>
                    </div>
