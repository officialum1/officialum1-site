...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    let articles: any[] = [];
    try {
        articles = await query("SELECT title, meta_description, keywords FROM knowledge_base WHERE slug = ?", [slug]) as any[];
    } catch (error) {
        console.error("Failed to load KB article metadata:", error);
    }
...
export default async function KBArticlePage({ params }: Props) {
    ...
    let articles: any[] = [];
    try {
        articles = await query("SELECT * FROM knowledge_base WHERE slug = ?", [slug]) as any[];
    } catch (error) {
    ...
    let related: any[] = [];
    try {
        related = await query("SELECT title, slug FROM knowledge_base WHERE category = ? AND id != ? LIMIT 3", [art.category, art.id]) as any[];
    } catch (error) {
        console.error("Failed to load related KB articles:", error);
    }
    ...
    try {
        ...
        console.error("Failed to update KB article views:", error);
    }
