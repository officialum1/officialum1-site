...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    let rows: any[] = [];
    try {
        rows = await query("SELECT title, content, meta_description, keywords FROM knowledge_base WHERE slug = ? AND is_published = 1", [slug]) as any[];
    } catch (error) {
        console.error("Failed to load help article metadata:", error);
    }
...
export default async function ArticlePage({ params }: Props) {
    ...
    let rows: any[] = [];
    try {
        rows = await query("SELECT * FROM knowledge_base WHERE slug = ? AND is_published = 1", [slug]) as any[];
    } catch (error) {
    ...
    try {
        ...
        console.error("Failed to update help article views:", error);
    }
