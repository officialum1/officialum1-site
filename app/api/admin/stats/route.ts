...
export async function GET(request: Request) {
    ...
    try {
        ...
        const wallets: any = { meezan: 0, ubl: 0, binance: 0, redotpay: 0, skrill: 0 };
        walletRows.forEach((row: any) => {
            const p = row.platform?.toLowerCase() || '';
            if (p.includes('meezan')) wallets.meezan = Number(row.balance);
        ...
        return NextResponse.json({
            ...
            totalProfit: 0,
            totalOrders: 0,
            wallets: { meezan: 0, ubl: 0, binance: 0, redotpay: 0, skrill: 0 },
            error: error.message
        });
    }
}
