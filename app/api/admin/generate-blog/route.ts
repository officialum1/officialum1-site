...
export async function POST(request: Request) {
    ...
    try {
        ...
        const settingsRes: any = await query("SELECT * FROM settings WHERE setting_key IN ('openaiKey', 'geminiKey', 'marketing_ai_model')");
        const settings: any = {};
        ...
        // Check for preferred model string in settings
        const preferredModel = settings.marketing_ai_model || 'openai';

        const useOpenAI = async () => {
            const prompt = `Write a professional, SEO-optimized ${isGuestPost ? 'guest post' : 'blog post'} about "${topic}". 
            ${isGuestPost ? 'IMPORTANT: This article will be published on an external site. Include exactly one natural, contextually relevant link to "https://officialum1.com" using anchor text related to high-quality gaming accounts or digital services.' : ''}
        ...
        if (preferredModel === 'gemini' && geminiKey) {
            try {
                const generated = await useGemini();
                title = generated.title; category = generated.category; excerpt = generated.excerpt; content = generated.content;
        ...
        // Try OpenAI if Gemini failed or was not preferred but is available
        if (!generationSuccess && openaiKey) {
            try {
                const generated = await useOpenAI();
                title = generated.title; category = generated.category; excerpt = generated.excerpt; content = generated.content;
                generationSuccess = true;
            } catch (e: any) {
                lastError = e.message;
        ...
        // Final fallback to Gemini if OpenAI failed and we haven't tried Gemini yet
        if (!generationSuccess && geminiKey && preferredModel !== 'gemini') {
            try {
                const generated = await useGemini();
                title = generated.title; category = generated.category; excerpt = generated.excerpt; content = generated.content;
                generationSuccess = true;
                ...
                lastError = e.message;
                console.error("Final Gemini fallback failed:", e.message);
            }
        }
