...
export async function POST(req: Request) {
    try {
        ...
        let prefix = 'ORD-';
        if (cartItems && Array.isArray(cartItems)) prefix = 'BULK-';
        if (membershipPlan) prefix = 'MEM-';
        ...
        if (membershipPlan) {
            ...
            product = { name: `${plan.name} Plan`, id: membershipPlan, platform: "Membership" };
        ...
        if (method === 'stripe') {
            const stripeSecret = (settings.stripeSecret || process.env.STRIPE_SECRET_KEY || '').trim();
            if (!stripeSecret) {
            ...
            params.append('success_url', `${origin}/order-success?orderId=${orderId}`);
            ...
            if (stripeData.url) {
                ...
                throw new Error(stripeData.error?.message || "Stripe initialization failed");
            }
        } else if (method === 'cryptomus') {
            const cryptoKey = (settings.cryptomusKey || process.env.CRYPTOMUS_API_KEY || '').trim();
            const cryptoId = (settings.cryptomusId || process.env.CRYPTOMUS_MERCHANT_ID || '').trim();

            if (!cryptoKey || !cryptoId) {
                console.error("Cryptomus Configuration Error: Missing API Key or Merchant ID", { hasKey: !!cryptoKey, hasId: !!cryptoId });
                throw new Error("Config Error: Cryptomus credentials missing.");
            }
            ...
            const payload = {
                ...
                url_return: `${origin}/order-success`,
            ...
            if (cryptoData.result?.url) {
                paymentUrl = cryptoData.result.url;
            } else {
                console.error("Cryptomus API Error:", cryptoData);
                throw new Error(cryptoData.message || "Cryptomus initialization failed");
            }
        } else if (method === 'binance') {
            const binanceKey = (settings.binanceKey || process.env.BINANCE_API_KEY || '').trim();
            const binanceSecret = (settings.binanceSecret || process.env.BINANCE_SECRET_KEY || '').trim();

            if (!binanceKey || !binanceSecret) {
                console.error("Binance Configuration Error: Missing API Key or Secret");
                throw new Error("Config Error: Binance credentials missing.");
            }
            ...
            if (binanceData.data?.checkoutUrl) {
                paymentUrl = binanceData.data.checkoutUrl;
            } else {
                console.error("Binance API Error:", binanceData);
                throw new Error(binanceData.errorMessage || "Binance initialization failed");
            }
