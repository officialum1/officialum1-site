        ...
        if (body.action === 'delete') {
            ...
            await query(
                `UPDATE products SET
                    ...
                    sale_price = ?, sale_ends_at = ?, bundle_items = ?, stock = ?,
                    category_id = ?,
            ...
            // Notify Google
            notifyGoogleIndexing(`https://officialum1.com/shop/${id}`).catch(e => console.error(e));
            ...
            const res: any = await query(
                `INSERT INTO products (
                    name, platform, price, description, image,
                    sale_price, sale_ends_at, bundle_items, stock, category_id,
                    seo_title, seo_description, focus_keyword, seo_keywords, canonical_url, robots, schema_type
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    name,
                    platform,
            ...
            // Notify Google
            if (res.insertId) {
                notifyGoogleIndexing(`https://officialum1.com/shop/${res.insertId}`).catch(e => console.error(e));
            }
        }
