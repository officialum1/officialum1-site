...
class MainProvider with ChangeNotifier {
  ...
  List<Z2UListing> get z2uListings => _z2uListings;
  ...
  bool get isLoadingZ2U => _isLoadingZ2U;
  ...
  Future<void> fetchZ2UListings() async {
    _isLoadingZ2U = true;
    notifyListeners();
    try {
      final res = await _api.get('/admin/z2u');
      if (res.statusCode == 200) {
        final List data = jsonDecode(res.body);
        _z2uListings = data.map((item) => Z2UListing.fromJson(item)).toList();
      }
    } catch (e) { 
      debugPrint('Error fetching Z2U: $e');
    } finally {
      _isLoadingZ2U = false;
      notifyListeners();
    }
