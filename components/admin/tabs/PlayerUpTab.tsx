...
export default function PlayerUpTab() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('OFFICIALUM1_PU_LOG', handleLog);
        return () => window.removeEventListener('OFFICIALUM1_PU_LOG', handleLog);
    }, []);
    ...
    const checkCookieStatus = async () => {
        try {
            ...
            fetch('/api/admin/playerup', {
                ...
                body: JSON.stringify({ action: 'cloud_bump_all', limit: 20 })
            }).catch(() => { });
    ...
    const [showBumpAllModal, setShowBumpAllModal] = useState(false);
    const [bumpAllFreq, setBumpAllFreq] = useState('Every 2 hours');

    const handleAutoBumpSmart = async (strategy: 'reddit_top50' | 'social_top50' | 'balanced') => {
        if (!confirm(`Apply Smart Bump Strategy: ${strategy}? This will optimize frequencies to avoid bans.`)) return;
        ...
        try {
            await fetch('/api/admin/playerup', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'smart_schedule', strategy })
            });

            modernAlert("Success", "Smart Strategy Applied! 🧠 Rankings Optimized.", "success");
            setShowBumpAllModal(false);
            fetchListings();
        } catch (e) {
            modernAlert("Failed", "Could not apply strategy.", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAutoBumpAll = async () => {
        if (!confirm(`Are you sure you want to enable Auto Bump for ALL ${listings.length} listings with frequency ${bumpAllFreq}?`)) return;
        ...
        try {
            await fetch('/api/admin/playerup', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'update_all', autoBump: true, frequency: bumpAllFreq })
            });

            modernAlert("Success", "All listings have been set to Auto Bump! 🚀", "success");
            setShowBumpAllModal(false);
            fetchListings();
            ...
            try {
                await fetch('/api/admin/playerup', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action: 'cloud_bump_all', limit: 5 })
                });
            } catch (e) { }

        } catch (e) {
            modernAlert("Failed", "Could not update all listings.", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleManualBump = async (listing: Listing) => {
        ...
        setTimeout(() => {
            ...
            fetchListings();
        }, 15000); // 15s delay to match extension load time (8s load + 5s action)
    };
...
(async () => {
    ...
    const frequencies = ['Never Stop', 'Every 5 seconds', 'Every 15 seconds', 'Every 30 seconds', 'Every 1 minute', 'Every 5 minutes', 'Every 10 minutes', 'Every 30 minutes', 'Every 1 hour', 'Every 2 hours', 'Every 6 hours', 'Every 12 hours', 'Every 24 hours'];
    ...
    useEffect(() => {
        fetchLogs();
        const interval = setInterval(fetchLogs, 15000); // Poll every 15s - reduced from 3s
    ...
    const handlePlayerUpAction = (type: 'sort' | 'delete') => {
        const adminKey = localStorage.getItem('admin_key');
        window.dispatchEvent(new CustomEvent('OFFICIALUM1_PLAYERUP_BATCH_ACTION', {
            detail: {
                actionType: type,
                pass: adminKey,
                url: "https://www.playerup.com/account/threads"
            }
        }));
        modernAlert("📡 Operation Dispatched", `Targeting PlayerUp Satellite... Action: ${type.toUpperCase()}`, "success");
    };
        ...
        <div className="FadeIn p-4">
            <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-6">
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={() => setShowBumpAllModal(true)} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-xl shadow-purple-600/20 transition-all flex items-center gap-2">
                        ⚡ Auto Bump All
                    </button>
                    <button onClick={() => {
            ...
            <div className="mb-8 glass-heavy rounded-xl border border-white/10 overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="h-48 overflow-y-auto p-4 font-mono text-[11px] space-y-1 custom-scrollbar bg-black/80">
                    {debugLog.length > 0 && debugLog.map((log, i) => (
                        <div key={`debug-${i}`} className="flex gap-3 text-cyan-400 border-l-2 border-cyan-500/30 pl-2">
                            <span className="text-gray-600 shrink-0">[{new Date().toLocaleTimeString([], { hour12: false })}]</span>
                            <span className="font-bold">🛰️ SATELLITE:</span>
                            <span>{log}</span>
                        </div>
                    ))}
                    {logs.length === 0 && debugLog.length === 0 ? (
                        <div className="text-gray-600 italic">Waiting for bump activity...</div>
                    ) : (
                        logs.map((log, i) => (
                            <div key={i} className="flex gap-3">
                                <span className="text-gray-500 shrink-0">[{new Date(log.date).toLocaleTimeString([], { hour12: false })}]</span>
            ...
            {showBumpAllModal && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[#111] border border-white/10 p-8 rounded-3xl w-full max-w-md shadow-2xl relative">
                        <h3 className="text-2xl font-bold text-white mb-2">⚡ Configure Auto Bump</h3>
                        <p className="text-gray-400 mb-6 text-sm">
                            Enable automatic bumping for <b>{listings.length} listings</b>. Select how often each thread should be bumped.
                        </p>

                        <div className="mb-6">
                            <label className="text-xs uppercase tracking-widest text-gray-500 font-bold mb-2 block">🧠 Smart Strategies</label>
                            <div className="flex flex-col gap-2">
                                <button
                                    onClick={() => handleAutoBumpSmart('reddit_top50')}
                                    className="bg-orange-500/10 hover:bg-orange-500 text-orange-400 hover:text-white border border-orange-500/20 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all text-left flex items-center justify-between group"
                                >
                                    <span>🏆 Reddit Top 50 Strategy</span>
                                    <span className="opacity-50 text-[10px] normal-case font-medium group-hover:text-white">Top 50 @ 2h, Rest @ 12h</span>
                                </button>
                                <button
                                    onClick={() => handleAutoBumpSmart('social_top50')}
                                    className="bg-blue-500/10 hover:bg-blue-500 text-blue-400 hover:text-white border border-blue-500/20 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all text-left flex items-center justify-between group"
                                >
                                    <span>⭐ Social Top 50 Strategy</span>
                                    <span className="opacity-50 text-[10px] normal-case font-medium group-hover:text-white">Top 50 @ 2h, Rest @ 12h</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="mb-6">
                            <label className="text-xs uppercase tracking-widest text-gray-500 font-bold mb-2 block">Standard Bulk Bump</label>
                            <select
                                value={bumpAllFreq}
                                onChange={(e) => setBumpAllFreq(e.target.value)}
                                className="w-full bg-black/50 border border-white/10 text-white rounded-xl px-4 py-3 outline-none focus:border-purple-500 transition-all text-sm font-medium"
                            >
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => setShowBumpAllModal(false)}
                            ...
                            <button
                                ...
                                className="flex-1 bg-purple-600 hover:bg-purple-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-purple-600/20 transition-all"
                            >
                                Start Engine 🚀
                            </button>
                        </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-white/5 text-[11px] uppercase tracking-[0.2em] text-gray-400 font-black">
                            ...
                            <th className="px-6 py-5 text-center">Daily Count</th>
                    ...
                    <tbody className="divide-y divide-white/5 text-sm">
                        {loading && listings.length === 0 ? (
                            ...
                            // Calculate Next Bump Logic
                            let nextBump = "Manual";
                            let nextBumpColor = "text-gray-500";
                            let isDue = false;

                            if (l.autoBump && l.frequency) {
                                ...
                                const lastTime = l.lastBumped ? new Date(l.lastBumped).getTime() : 0;
                                const nextTime = lastTime + interval;
                                const now = Date.now();
                                const diff = nextTime - now;

                                if (l.dailyBumpCount >= 4) {
                                    nextBump = "Limit Reached";
                                    nextBumpColor = "text-orange-500";
                                } else if (diff <= 0) {
                                    nextBump = "⚡ DUE NOW";
                                    nextBumpColor = "text-yellow-400 animate-pulse";
                                    isDue = true;
                                } else {
                                    const hours = Math.floor(diff / (1000 * 60 * 60));
                                    const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
                                    nextBump = `Next: ${hours}h ${minutes}m`;
                                    nextBumpColor = "text-blue-400";
                                }
                            } else if (!l.autoBump) {
                                nextBump = "Paused";
                                nextBumpColor = "text-gray-600";
                            }
                            ...
                            return (
                                <tr key={l.id} className={`hover:bg-white/[0.02] transition-colors ${selectedIds.has(l.id) ? 'bg-blue-500/5' : ''}`}>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        ...
                                        <div className={`text-[9px] font-bold mt-1 uppercase ${nextBumpColor}`}>{nextBump}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <button
                                    ...
                                    <td className="px-6 py-4 text-center whitespace-nowrap">
                                        <div className={`text-xs font-mono font-bold ${l.lastBumpStatus === 'failed' ? 'text-red-400' : (l.lastBumpStatus === 'limit_reached' ? 'text-orange-400' : 'text-emerald-400')}`}>
                                            {l.lastBumped ? new Date(l.lastBumped).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '[New]'}
                                        </div>
