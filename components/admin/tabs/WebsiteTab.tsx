...
export default function WebsiteTab({
    ...
    const blogSeoTitle = (blogForm.meta_title || blogForm.title || '').trim();
    const blogSeoDescription = (blogForm.meta_description || blogForm.excerpt || '').trim();
    const blogWordCount = (blogForm.content || '').trim().split(/\s+/).filter(Boolean).length;
    const blogSeoChecks = [
        blogSeoTitle.length >= 35 && blogSeoTitle.length <= 65,
        blogSeoDescription.length >= 120 && blogSeoDescription.length <= 160,
        Boolean((blogForm.focus_keyword || '').trim()),
        blogWordCount >= 700,
        (blogForm.robots || 'index,follow').includes('index'),
    ];
    const blogSeoScore = Math.round((blogSeoChecks.filter(Boolean).length / blogSeoChecks.length) * 100);
                    ...
                    <div className="glass" style={{ padding: '2.5rem', borderRadius: '20px', border: '1px solid rgba(0,255,136,0.1)' }}>
                        ...
                        <form onSubmit={handleBlogSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                            ...
                            <div>
                                ...
                                <select value={blogForm.category || 'SEO'} onChange={e => setBlogForm({ ...blogForm, category: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }} required>
                            ...
                            <div style={{ gridColumn: 'span 2', background: 'rgba(20,108,120,0.10)', border: '1px solid rgba(20,108,120,0.28)', padding: '1.25rem', borderRadius: '14px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                                    <div>
                                        ...
                                        <p style={{ color: '#9fb1b8', fontSize: '0.75rem', marginTop: '0.25rem' }}>Controls Google title, meta description, focus keyword, canonical, robots and article schema.</p>
                                    </div>
                                    <div style={{ color: blogSeoScore >= 80 ? '#00ff88' : blogSeoScore >= 55 ? '#ffaa00' : '#ff6b6b', fontWeight: 900 }}>
                                        SEO {blogSeoScore}%
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>SEO Title</label>
                                        <input placeholder="Google title under 60 characters" value={blogForm.meta_title || ''} onChange={e => setBlogForm({ ...blogForm, meta_title: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }} />
                                        <p style={{ color: '#7f9299', fontSize: '0.7rem', marginTop: '0.25rem' }}>{blogSeoTitle.length}/60 recommended</p>
                                    </div>
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Focus Keyword</label>
                                        <input placeholder="e.g. guest posting" value={blogForm.focus_keyword || ''} onChange={e => setBlogForm({ ...blogForm, focus_keyword: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }} />
                                    </div>
                                    <div style={{ gridColumn: 'span 2' }}>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Meta Description</label>
                                        <textarea placeholder="Google snippet under 155 characters" value={blogForm.meta_description || ''} onChange={e => setBlogForm({ ...blogForm, meta_description: e.target.value })} className="input-field" style={{ width: '100%', height: '80px', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff', resize: 'vertical' }} />
                                        <p style={{ color: '#7f9299', fontSize: '0.7rem', marginTop: '0.25rem' }}>{blogSeoDescription.length}/155 recommended</p>
                                    </div>
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Extra Keywords</label>
                                        <input placeholder="comma, separated, keywords" value={blogForm.seo_keywords || ''} onChange={e => setBlogForm({ ...blogForm, seo_keywords: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }} />
                                    </div>
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Canonical URL</label>
                                        <input placeholder="Leave blank for default blog URL" value={blogForm.canonical_url || ''} onChange={e => setBlogForm({ ...blogForm, canonical_url: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }} />
                                    </div>
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Robots Meta</label>
                                        <select value={blogForm.robots || 'index,follow'} onChange={e => setBlogForm({ ...blogForm, robots: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }}>
                                    ...
                                    <div>
                                        <label style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block', fontWeight: '500' }}>Schema Type</label>
                                        <select value={blogForm.schema_type || 'BlogPosting'} onChange={e => setBlogForm({ ...blogForm, schema_type: e.target.value })} className="input-field" style={{ width: '100%', padding: '1rem', background: '#0a0a0a', border: '1px solid #333', borderRadius: '12px', color: '#fff' }}>
                            ...
                            <div style={{ gridColumn: 'span 2' }}>
                                ...
                                <p style={{ color: '#7f9299', fontSize: '0.75rem', marginTop: '0.35rem' }}>{blogWordCount} words. Aim 700+ for stronger informational posts.</p>
                            </div>
