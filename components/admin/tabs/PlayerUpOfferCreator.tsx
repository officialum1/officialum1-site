...
export default function PlayerUpOfferCreator() {
    ...
    useEffect(() => {
        ...
        else setPosterCategoryUrl("https://www.playerup.com/forums/reddit-accounts.407/create-thread");
        handleGenerateDetails(true);
        fetchListings();

        // 🛰️ EXTENSION FEEDBACK LISTENER
        const handleSuccess = (e: any) => {
            console.log("🚀 Post Success Event Received!", e.detail);
            setLoading(false);
            fetchListings();
            modernAlert("Post Confirmed!", "Thread URL captured and synced to database.", "success");
        };
        window.addEventListener('OFFICIALUM1_POST_SUCCESS', handleSuccess);

        return () => window.removeEventListener('OFFICIALUM1_POST_SUCCESS', handleSuccess);
    }, []);
...
[COLOR=white]💬 LINE ID:[/COLOR] [URL=https://line.me/ti/p/officialum1][COLOR=deepskyblue]📱 officialum1[/COLOR][/URL]
...
[COLOR=green]Thank you for choosing Officialum1! 🙏[/COLOR][/CENTER]`;
    ...
    const handleTurboPost = async () => {
        ...
        setLoading(true); // Button will say SENDING TO PLAYERUP...
        try {
            ...
            // Safety timeout in case extension crashes/fails silently
        ...
        <div className="FadeIn p-4">
            <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-6">
                ...
                <button
                    ...
                    disabled={fetching}
                    className="bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl border border-white/10 text-xs transition-all flex items-center gap-2"
                >
                    {fetching ? '🔄 Updating...' : '🔄 Refresh List'}
                </button>
            </div>
            ...
            {/* LISTING HISTORY */}
            <div className="mt-12 bg-gray-900/50 border border-white/5 rounded-[40px] p-8 backdrop-blur-xl">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-white font-bold text-xl flex items-center gap-2">
                        📋 Recently Created & Synced Listings
                    </h3>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-white/5">
                                <th className="pb-4 text-[10px] text-gray-500 uppercase font-black">Platform</th>
                                <th className="pb-4 text-[10px] text-gray-500 uppercase font-black">Title</th>
                                <th className="pb-4 text-[10px] text-gray-500 uppercase font-black">Status</th>
                                <th className="pb-4 text-[10px] text-gray-500 uppercase font-black">Last Bumped</th>
                                <th className="pb-4 text-[10px] text-gray-500 uppercase font-black px-4 text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {listings.slice(0, 10).map((item) => (
                                <tr key={item.id} className="group hover:bg-white/5 transition-all">
                                    <td className="py-4">
                                        <span className="bg-white/5 border border-white/10 px-3 py-1 rounded-full text-[10px] font-bold text-gray-400">{item.platform}</span>
                                    </td>
                                    <td className="py-4">
                                        <div className="text-sm font-medium text-white group-hover:text-orange-400 transition-colors truncate max-w-md">{item.title}</div>
                                        <div className="text-[10px] text-gray-600 truncate max-w-xs">{item.url}</div>
                                    </td>
                                    <td className="py-4">
                                        <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${item.status === 'Active' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                                            }`}>
                                            {item.status}
                                        </span>
                                    </td>
                                    <td className="py-4 text-xs text-gray-500">
                                        {item.lastBumped ? new Date(item.lastBumped).toLocaleString() : 'Never'}
                                    </td>
                                    <td className="py-4 text-right px-4">
                                        <a
                                            href={item.url}
                                            target="_blank"
                                            className="bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 px-4 py-2 rounded-xl text-[10px] font-bold border border-orange-500/20 transition-all inline-block"
                                        >
                                            View Thread
                                        </a>
                                    </td>
                            ...
                            {listings.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="py-20 text-center text-gray-600 italic text-sm">
                                        No recent postings found. Try syncing or creating an offer.
                                    </td>
                                </tr>
                            )}
                        </tbody>
